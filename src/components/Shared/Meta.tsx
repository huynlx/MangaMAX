...
const Meta: React.FC<any> = ({ item }) => {
  ...
  return (
    <>
      ...
      {item.updateAt &&
        <small data-time={item.updateAt} className='lg:leading-[1.1] px-2 md:py-1 rounded-full absolute bg-nav/[.8] md:font-semibold text-white bottom-1 left-1 whitespace-nowrap text-ellipsis overflow-hidden max-w-[80%] 2xl:w-[auto]'>{item.updateAt}</small>}
