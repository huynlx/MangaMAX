...
const getHome = async (
  ...
  const handleData = () => {
    return htmls.map((source, index) => {
      ...
      const items = dom
        .querySelectorAll(".ModuleContent .items .item")
        .map((item) => {
          let elStatus = item
            .querySelectorAll(".message_main p")
            .find((el) =>
              el.querySelector("label")?.innerText.includes("Tình trạng:")
            );
          let status = elStatus?.childNodes[2]
            ? elStatus?.childNodes[2]?.textContent.includes("Đang")
              ? "ONGOING"
              : "COMPLETED"
            : "ONGOING";
          let cover = item
      ...
      const currentPage = +dom.querySelector("ul.pagination > li.active > a")
        ?.innerText!;

      return {
        name: Object.keys(sections)[index],
        items,
        hasNextPage: items.length > 0 ? hasNextPage : false,
        currentPage,
      };
    });
  };
