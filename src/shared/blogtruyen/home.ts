  ...
  const handleData = () => {
    if (type === "browse") {
      ...
      return htmls.map((source, index) => {
        ...
        const items = dom
          .querySelectorAll(".list p:not(:first-child)")
          .map((item) => {
            const cover = item.nextElementSibling
              .querySelector("img")
              ?.getAttribute("src")
              ?.replace("150x", "300x300");

            return {
              title: decodeHTMLEntity(item.querySelector("a")?.innerText!),
              cover: cover,
              chapter:
                item.querySelector("span:nth-child(2)")?.innerText +
                  " chương" ?? "",
              slug: item
                .querySelector("a")
                ?.getAttribute("href")
      ...
      return htmls.map((source, index) => {
        ...
        const items = dom
          .querySelectorAll(".gridview .row")[0]
          ?.querySelectorAll(".item")
          ?.map((item) => {
            const cover = item.querySelector(".image img")?.getAttribute("src");
            const time = item
              .querySelector(".chapter .timesince")
              ?.getAttribute("data-date");

            return {
              title: decodeHTMLEntity(
                item.querySelector("figcaption h3 a")?.innerText.trim()
              ),
              cover: cover,
              chapter:
                item.querySelector(".chapter a")?.getAttribute("title") ?? "",
              slug: item
        ...
        const lastPage = Math.max(...pages);
        const hasNextPage = +page !== lastPage;
        const currentPage = +dom
          .querySelector("ul.pagination > li > select > option:checked")
          ?.innerText.split(" ")[1]!;
