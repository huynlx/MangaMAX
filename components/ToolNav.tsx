...
const ToolNav = () => {
  ...
  return (
    <div className="bg-nav w-auto justify-center items-center gap-5 flex bg-opacity-80 backdrop-blur-xl px-5 py-2 rounded-full fixed bottom-5 left-2/4 transform -translate-x-2/4">
      <MdHistory title='History' onClick={() => Router.push('/recents')} size={44} className='hover:text-white duration-300' />
      <span title='Bookmarks' onClick={() => Router.push('/bookmarks')} className='hover:border-white border-[3px] p-[6px] rounded-full hover:text-white duration-300'><BsFillBookmarkFill /></span>
