...
const ToolNav: FC = () => {
  ...
  return (
    <div className={`bg-nav scale-75 lg:scale-100 w-auto justify-center items-center gap-5 flex bg-opacity-80 backdrop-blur-xl px-5 py-2 rounded-full fixed bottom-5 left-2/4 transform -translate-x-2/4 z-[6] overflow-hidden transition-width`}>
      {
        !close && (
          <>
            ...
            <FiArrowUp title='Top' onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} size={37} className='-ml-3 hover:text-white duration-300 hidden lg:block' />
            <FiArrowUp title='Top' onClick={() => window.scrollTo({ top: 0 })} size={37} className='-ml-3 hover:text-white duration-300 lg:hidden' />
