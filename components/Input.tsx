...
const Input: React.FC<any> = ({ isActive }) => {
  ...
  return (
    <form
      className={`${isActive ? 'flex' : 'hidden'} lg:flex border-accent rounded-full overflow-hidden h-10`}
      onSubmit={handleFormSubmit}
      id='myForm'
    >
      <button aria-label="Search" type='submit' className="bg-accent flex-shrink-0 flex justify-center items-center w-auto pl-3">
      ...
      <input
        type="search"
        className="bg-accent flex-grow text-white outline-none px-3 text-[17px]"
        id='keyword'
        placeholder="Search"
        value={inputValue}
